...
namespace geometry {
    /** Forward Segment class declaration
     */
    class Segment;
...
//		~IntersectionPointStruct() = default;
	...
	class Point{
		...
		private:
			...
			/** list of all the segments this point is an endpoint of (belongs to)
			*/
			std::set<std::shared_ptr<Segment> > segList_;
			
			/** Individual index of this Point object
			*/
			unsigned int idx_;
        
			/**	Global counter of Point objects created
			 */
