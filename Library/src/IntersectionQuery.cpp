...
vector<shared_ptr<PointStruct> > IntersectionQuery::findAllIntersectionsSmart(){
    ...
    for(auto itr = eventQueue.begin(); itr != eventQueue.end(); itr++){
        ...
        auto segToInsert = *currSegSet.begin();
...
//                    if (priorityItr != prioritySegSet.end()){prioritySegSet.erase(priorityItr);}
            ...
            if(prioritySegSet.size() == 1){
                ...
                //get iterator to the current Segment from the priority segment set
                auto priorityCurrSegItr = prioritySegSet.find(segToInsert);
//                for(auto it = prioritySegSet.begin(), previt = it++ ; it != prioritySegSet.end(); ++it, ++previt){
                    //auto prevSeg = *previt;  //this is current seg
                //dereference the pointer priorityCurrSegItr
                auto currentSeg = *priorityCurrSegItr;
                //dereference the pointer on the left of the priorityCurrSegItr
                auto compareSeg = *priorityCurrSegItr--;
                //loop to compare current Seg with the seg on right and on left
                for(int i = 0; i<2; i++){
                    //call vector's iterator to check if the currentSeg and prevSeg are already compared or not
                    vector< pair<shared_ptr<Segment>, shared_ptr<Segment> > >::iterator findPairItr = std::find(intersectionCheckSegs.begin(),                        intersectionCheckSegs.end(),make_pair(compareSeg,currentSeg) );
                    if (findPairItr != intersectionCheckSegs.end()){
                        ...
                        shared_ptr<PointStruct> smartIntersectionPt = compareSeg->findIntersection(*(currentSeg));
                        if (smartIntersectionPt != nullptr){
                            smartIntersectVect.push_back(smartIntersectionPt);
                            addEvent(eventQueue, smartIntersectionPt);
                            intersectionCheckSegs.push_back(make_pair(compareSeg,currentSeg));
                        }
                    }
                    compareSeg = *priorityCurrSegItr++;
                }
                //}
            }
        }else{ //if currPoint is an intersection point then execute this code snippet
            //it++ will increment the value of it, but return the original value that it held before being incremented.
            for(auto it = currSegSet.begin(), previt = it++ ; it != currSegSet.end(); ++it, ++previt){
                auto currentSeg = *previt;  //this is current seg
                auto nextSeg = *it;   //and this is next
                //check if these 2 segments are swapped before or not
                vector< pair<shared_ptr<Segment>, shared_ptr<Segment> > >::iterator findPairItr2 = std::find(swappedSegs.begin(),swappedSegs.end(),make_pair(currentSeg,nextSeg) );
